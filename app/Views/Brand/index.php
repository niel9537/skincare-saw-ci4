...
<div class="card mt-3 shadow-sm">
    ...
    <div class="card-header py-3 d-flex justify-content-end align-items-center">
        ...
        <a href="<?= base_url('/admin/brand/tambah/') ?>" class="btn btn-sm btn-primary <?= $_SESSION['role'] == 1 ? '' : 'd-none' ?>">
                        ...
                        <?php foreach ($brand as $row) : ?>
                            <tr>
                                ...
                                <td class="<?= $_SESSION['role'] == 1 ? '' : 'd-none' ?>">
                                    <form action="/admin/brand/edit/<?= $row['id_brand'] ?>" method="get" class="d-inline">
                                        <?= csrf_field() ?>
                                        <input type="hidden" name="_method" value="GET">
                                        <button type="submit" class="btn btn-sm btn-warning"><i class="bi bi-pencil-square"></i></button>
                                    </form>
                                    <form action="/admin/brand/hapus/<?= $row['id_brand'] ?>" method="post" class="d-inline">
                                        <?= csrf_field() ?>
